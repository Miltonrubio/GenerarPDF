...
async function generatePDF(curso, nombres, apellidos, email, direccion, telefono, hijos, numeroHijos, discapacidad, discapacidadDesc) {
    const image = await loadImage("tarjeta_control.png");
    const signatureImage = signaturePad.toDataURL();

    const pdf = new jsPDF('p', 'pt', 'legal');

    pdf.addImage(image, 'PNG', 0, 0, 500, 950);
    ...
    pdf.setFontSize(15);
    pdf.text(nombres, 180, 240);
    pdf.text(apellidos, 250, 240);
